...
var button1 = document.getElementById("iss_but").addEventListener("click", fun2)

function fun2(event) {
    event.preventDefault();
    var search = document.getElementById('bor');
    var data = {
        type : "book",
        id: search.value,
    ...
    // alert(data["input"]);
    var xhr = new XMLHttpRequest();
    xhr.open('POST', 'http://127.0.0.1:3000/document_lending');
    xhr.setRequestHeader('Content-Type', 'application/json');
    xhr.onreadystatechange = function() {
    if (xhr.readyState === 4 && xhr.status === 200) {
        // Handle the response data
